...
func addWs2(ws []interface{}) (addWs []interface{}) {
	...
	///////////////
	//m53 := make(map[string]interface{})
	//m53["text"] = "流"
	//m53["type"] = float64(0)
	//ws = append(ws, m53)
	//
	//m54 := make(map[string]interface{})
	//m54["text"] = "流"
	//m54["type"] = float64(2)
	//ws = append(ws, m54)
	//
...
func markSameZeroSli(zeroSliCache []interface{}, normalSliCache []interface{}) {
	// multi words is same
...
func markRepeat(ws []interface{}) (markWs []interface{}) {
	...
	for j := range ws {
		...
		case 0:
			...
			if len(zeroSliCache) > len(normalSliCache) && (len(zeroSliCache) != 0 && len(normalSliCache) != 0) {
				markSameZeroSli(zeroSliCache, normalSliCache)
